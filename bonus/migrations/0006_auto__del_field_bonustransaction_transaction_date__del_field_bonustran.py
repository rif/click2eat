...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'BonusTransaction.transaction_date'
        db.delete_column('bonus_bonustransaction', 'transaction_date')

        # Deleting field 'BonusTransaction.user'
        db.delete_column('bonus_bonustransaction', 'user_id')

        # Deleting field 'BonusTransaction.from_user'
        db.delete_column('bonus_bonustransaction', 'from_user_id')


    def backwards(self, orm):
        
        # User chose to not deal with backwards NULL issues for 'BonusTransaction.transaction_date'
        raise RuntimeError("Cannot reverse this migration. 'BonusTransaction.transaction_date' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'BonusTransaction.user'
        raise RuntimeError("Cannot reverse this migration. 'BonusTransaction.user' and its values cannot be restored.")

        # Adding field 'BonusTransaction.from_user'
        db.add_column('bonus_bonustransaction', 'from_user', self.gf('django.db.models.fields.related.ForeignKey')(related_name='bonus_given_set', null=True, to=orm['auth.User'], blank=True), keep_default=False)
